...
import com.moutamid.tubeking.databinding.ActivitySignInBinding;
    ...
    private void updateUI(FirebaseUser user) {
        ...
        Constants.databaseReference().child(Constants.USER).child(user.getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (!snapshot.exists()) {
                    UserDetails userDetails = new UserDetails(user.getDisplayName(), user.getEmail(), 1000, false, user.getPhotoUrl().toString());
